...
Array<Surface> Surface::FromSpriteSheet(const char* filename, int tilesize, int wcount, int hcount, IntArray ids) {
	Array<Surface> result(ids.GetLength());
	// use stb_image to load the image file
	int numComponents;
	int widthPixels = wcount * tilesize;
	int heightPixels = hcount * tilesize;
	unsigned char* data = stbi_load(filename, &widthPixels, &heightPixels, &numComponents, 0);
	if (!data) {
		printf("Failed to read from sprite sheet: data is null");
		return result; // load failed
	}
	//printf("%d", ids.GetLength());
	for (int surfi = 0; surfi < ids.GetLength(); surfi++) {
		int id = ids[surfi];
		int pixx = (id % wcount)* tilesize;
		int pixy =  (id / wcount)* tilesize;
		int pixel = pixy* widthPixels + pixx;
		uint* spixels = (uint*)MALLOC64(tilesize * tilesize * sizeof(uint));
		int writeindex = 0;
		for (int y = 0; y < tilesize; y++) {
			for (int x = 0; x < tilesize; x++) {
				int readindex = (y + pixy) * widthPixels + (x + pixx);
				spixels[writeindex] =
					(data[readindex * 4 + 0] << 16) +
					(data[readindex * 4 + 1] << 8) +
					(data[readindex * 4 + 2]);
				writeindex++;
			}
		}
		result[surfi] = Surface(tilesize, tilesize, spixels);
	}
	// free stb_image data
	stbi_image_free(data);
	return result;
